...
const gpuFile = require('../../classroom/modules/00-pick-your-lab/catalog/gpu.json');
assert.deepStrictEqual(gpuFile.map(function (item) { return item.id; }), wiz.gpuCatalog().map(function (item) { return item.id; }));
assert.ok(tightLab.model.fit.cpu.plain.indexOf('Zen 4') !== -1);
const markets = wiz.markets().map(function (market) { return market.name; });
assert.ok(markets.indexOf('Jawa') !== -1);
assert.ok(markets.indexOf('Amazon') !== -1);
assert.ok(markets.indexOf('PCPartPicker') !== -1);
const offers = wiz.marketOffers('RTX 5070 Ti');
assert.ok(offers.every(function (offer) { return offer.price == null && offer.checked_at == null && offer.url; }));
assert.ok(offers.some(function (offer) { return offer.market === 'Jawa' && offer.condition === 'used'; }));
assert.ok(offers.some(function (offer) { return offer.market === 'Newegg' && offer.condition === 'new'; }));
const ti = wiz.gpuCatalog().filter(function (item) { return item.id === 'rtx-5070ti-16'; })[0];
assert.strictEqual(ti.architecture, 'Blackwell');
assert.strictEqual(ti.vram_gb, 16);
assert.strictEqual(ti.memory, 'GDDR7');
assert.strictEqual(ti.watts, 300);
assert.strictEqual(ti.psu, 750);
const paths = wiz.upgradePaths('rtx-3060-12');
assert.strictEqual(paths.filter(function (row) { return row.role === 'Value'; })[0].product.id, 'rtx-4070-12');
assert.strictEqual(paths.filter(function (row) { return row.role === 'Balanced'; })[0].product.id, 'rtx-5070ti-16');
assert.strictEqual(paths.filter(function (row) { return row.role === 'AI / VRAM'; })[0].product.id, 'rtx-3090-24');
assert.strictEqual(paths.filter(function (row) { return row.role === 'Performance'; })[0].product.id, 'rtx-5080-16');
const from3060 = wiz.gpuCatalog().filter(function (item) { return item.id === 'rtx-3060-12'; })[0];
const impact = wiz.gpuSwapImpact(from3060, ti, 65, 650);
assert.deepStrictEqual(impact.vram, [12, 16]);
assert.deepStrictEqual(impact.watts, [170, 300]);
assert.strictEqual(impact.formulaOk, true);
assert.strictEqual(impact.vendorOk, false);
assert.ok(wiz.gpuScores(wiz.gpuCatalog().filter(function (item) { return item.id === 'rtx-3090-24'; })[0]).ai > wiz.gpuScores(ti).ai);
const gpuLab = wiz.labBreakdown(base({ side: 'linux', role: 'everyday', job: ['games'], gpu: 16, games: 'some', flavor: 'mint' }));
assertLevels(gpuLab);
assert.ok(walkTree(gpuLab, []).some(function (node) { return node.title === 'RTX 5070 Ti 16GB' && node.level === 'L5' && node.product && node.product.architecture === 'Blackwell'; }));
